...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query(
        value = "select reservation from Reservation reservation left join fetch reservation.course left join fetch reservation.gymService left join fetch reservation.registeredBy",
    ...
    @Query(
        "select reservation from Reservation reservation left join fetch reservation.course left join fetch reservation.gymService left join fetch reservation.registeredBy"
    )
    List<Reservation> findAllWithToOneRelationships();

    @Query(
        "select reservation from Reservation reservation left join fetch reservation.course left join fetch reservation.gymService left join fetch reservation.registeredBy where reservation.id = :id"
    )
