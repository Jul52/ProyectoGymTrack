...
public class CourseRepositoryWithBagRelationshipsImpl implements CourseRepositoryWithBagRelationships {
    ...
    public Optional<Course> fetchBagRelationships(Optional<Course> course) {
        return course.map(this::fetchZones);
    }
    ...
    public List<Course> fetchBagRelationships(List<Course> courses) {
        return Optional.of(courses).map(this::fetchZones).orElse(Collections.emptyList());
    }

    Course fetchZones(Course result) {
        return entityManager
            .createQuery("select course from Course course left join fetch course.zones where course.id = :id", Course.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Course> fetchZones(List<Course> courses) {
        ...
        List<Course> result = entityManager
            .createQuery("select course from Course course left join fetch course.zones where course in :courses", Course.class)
