...
public interface MachineRepository extends JpaRepository<Machine, Long> {
    ...
    @Query(
        value = "select machine from Machine machine left join fetch machine.admin",
        countQuery = "select count(machine) from Machine machine"
    )
    Page<Machine> findAllWithToOneRelationships(Pageable pageable);

    @Query("select machine from Machine machine left join fetch machine.admin")
    List<Machine> findAllWithToOneRelationships();

    @Query("select machine from Machine machine left join fetch machine.admin where machine.id =:id")
