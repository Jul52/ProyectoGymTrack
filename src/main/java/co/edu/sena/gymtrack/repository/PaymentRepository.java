...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query(
        value = "select payment from Payment payment left join fetch payment.paymentMethod left join fetch payment.registeredBy",
        countQuery = "select count(payment) from Payment payment"
    )
    Page<Payment> findAllWithToOneRelationships(Pageable pageable);

    @Query("select payment from Payment payment left join fetch payment.paymentMethod left join fetch payment.registeredBy")
